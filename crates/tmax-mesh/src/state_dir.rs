...
const DEFAULT_STATE_DIR: &str = ".local/state/tmax";

/// Return the tmax state directory path.
///
/// Priority: `$TMAX_STATE_DIR` env var, then `~/.local/state/tmax`.
pub fn default_state_dir() -> Result<PathBuf> {
    if let Ok(dir) = std::env::var("TMAX_STATE_DIR") {
